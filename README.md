...
**Исправление 4 — fallback на прямую загрузку при ошибке скачивания тела подписки через прокси:**
Проверка доступности (`wait_for_subscription_connectivity`) могла проходить успешно, а само скачивание тела подписки (`download_subscription`) через прокси — проваливаться после всех попыток (например, прокси не успевает обработать TLS-туннель за таймаут). В лог писалось `[error] Subscription body download failed for section '<section>' after retries` и `Subscription download failed after 3 attempts: host=...`, хотя у роутера есть прямой доступ к серверу подписки. Теперь `download_subscription_into_cache()` в этом случае делает одну дополнительную попытку скачать подписку напрямую (без прокси, без ожидания между попытками, таймаут 10 с). При успехе пишется `[info] ... succeeded directly after proxy failure` и обновление продолжается как обычно; если не удалась и прямая попытка — как и раньше, `[error] ... failed after retries`. Исправление действует везде, где используется `download_subscription_into_cache()`: подготовка кэша при старте, генерация конфига и `subscription_update` (ручной и по cron).
...
## Гонка между стартом и BadWAN-перезагрузкой при загрузке роутера (v0.7.43)

**Проблема:** При `enable_badwan_interface_monitoring=1` (включено по умолчанию) podkop регистрирует `procd_add_interface_trigger("interface.*.up", ..., "/etc/init.d/podkop reload")`. Интерфейс WAN поднимается при **каждой** загрузке роутера, поэтому через `badwan_reload_delay` (по умолчанию 2 с) после старта запускался `podkop reload`, который пересекался по времени с ещё выполняющимся стартовым `podkop start`. Два параллельных процесса формировали разные временные конфиги sing-box, и более поздний из них перезапускал sing-box посреди инициализации (`FATAL: start service: context canceled`). В результате после каждой перезагрузки FakeIP-таблица не успевала прогрузиться: диагностика показывала «FakeIP проверки: Проверки не выполнены», «DNS роутера не проходит через sing-box», «Браузер не использует FakeIP» — пока не выполнялся ручной `podkop restart`.

**Исправление:** Добавлена функция `acquire_lifecycle_lock()` (`flock` на `/var/run/podkop.lock`, fd 9), которой обёрнуты команды `start`, `stop`, `restart`, `reload` и `subscription_update` в диспетчере `/usr/bin/podkop`. Теперь BadWAN-перезагрузка дожидается завершения стартового `start`, а затем выполняет чистый `stop`+`start` против уже полностью поднятого sing-box — аналогично ручному `podkop restart`, который и раньше решал проблему. BadWAN-мониторинг продолжает работать как раньше (самолечение при реальных проблемах с WAN), просто больше не гоняется со стартом.

Побочный эффект: фоновая `list_update &`, запущенная стартовым `start`, наследует fd 9 и держит лок до своего завершения — поэтому BadWAN-reload ждёт не только сам `start`, но и его фоновое обновление списков. Из-за этого общее время «успокоения» после перезагрузки выросло (~90 c вместо мгновенного, но «битого» состояния), зато состояние корректное без ручных действий.

## Hysteria2-серверы из подписки могли отбраковываться из-за uTLS fingerprint

**Проблема:** При разборе ссылок вида `hysteria2://`/`hy2://` из подписки (base64 link-list формат) параметр `fp` (uTLS fingerprint, обычно унаследованный провайдером от соседних vless-ссылок) безусловно прокидывался в `tls.utls.fingerprint` итогового outbound'а. sing-box-extended отклоняет Hysteria2 (QUIC) outbound с заданным uTLS fingerprint ошибкой «unsupported usage for uTLS» (подтверждено вживую на роутере), `sing-box check` для такого outbound'а проваливается, и сервер тихо пропускается с `warn` в логе — независимо от выбранного User-Agent подписки.

**Исправление:** Для схем `hysteria2`/`hy2` параметр `fp` больше не передаётся в `tls.utls`, аналогично тому, как это уже было сделано в парсере Happ-формата подписки (`_sing_box_cf_add_subscription_outbounds_from_xray_config_list`).
...
- Автоматический разбор Happ-формата подписки (JSON-массив полных клиентских конфигов Xray) — добавляет vless (в т.ч. XHTTP/reality/tls) и Hysteria2 серверы
- User-Agent при скачивании подписки по умолчанию — Happ-подобный (`Happ/4.10.2/ios/...`) вместо классического `singbox/<версия>`: у провайдеров вроде Remnawave это наиболее полный из форматов — единственный, отдающий Hysteria2, плюс vless с XHTTP. Настраивается опцией `subscription_user_agent` (`v2rayN/9.99` вернёт base64 список ссылок — пригодится, если у вашего провайдера в Happ-формате не хватает shadowsocks/trojan/socks; `singbox` вернёт классический формат, либо можно указать произвольный UA)
- Секции типа Exclusion больше не отображаются на дашборде пустой нерабочей карточкой
- Если провайдер отдаёт подписку в gzip независимо от заголовков запроса, она корректно распаковывается перед разбором
...
- `User-Agent: Happ/4.10.2/ios/...` (см. выше)
