...
Из podkop-evolution унаследована только базовая возможность указать ссылку подписки от провайдера (`proxy_config_type='subscription'` + `subscription_url` + интервал автообновления). Всё остальное в этом разделе — доработки Podkop Forge:

- Все серверы из подписки автоматически появляются в дашборде, с автоматическим выбором лучшего по задержке (URLTest) и ручным переключением между ними
