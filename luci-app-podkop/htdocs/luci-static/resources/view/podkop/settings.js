...
function createSettingsContent(section) {
  ...
  o = section.option(
    form.Flag,
    "dns_failover_on_local",
    _("Переключаться с локального DNS при перехвате"),
    _("Если DNS указан на локальный сервер (AdGuard Home, Pi-hole), автопереключение обычно пропускается — локальный сервер провайдер заблокировать не может. Но заблокировать можно его апстрим: тогда локальный сервер жив и отвечает, а раздаёт подменённые ответы. С этой галкой podkop при обнаружении перехвата 53 порта переключится на шифрованный DNS в обход локального сервера. Ценой блокировки рекламы, поэтому по умолчанию выключено."),
  );
  o.default = "0";
  o.rmempty = false;
  ...
  o = section.option(
    ...
    _("Нужно, только если на устройстве в LAN DoH или DoT прописан вручную — в браузере, в Windows 11 или в «Приватном DNS» Android. Такой клиент резолвит имена мимо роутера, FakeIP не выдаёт адрес, и сайты тихо перестают идти через туннель. Автоматическое включение DoH в браузерах podkop подавляет и без этой галки. Правило режет 853 порт целиком (DoT/DoQ) и список известных публичных DoH-серверов на 443. Собственный DNS роутера не затрагивается. Клиент с личным DoH-сервером под правило не попадёт."),
  );
  o.default = "0";
  o.rmempty = false;
